...
{
	public partial class MultiTimedAnimationTool : Form
	{
		...
		public MultiTimedAnimationTool(Form parent)
		{
			m_parent = parent;
		...
		{
			AddAnimation("Anim" + m_animations.Count);
		}
		...
		private void InvertAnimationList_Click(object sender, EventArgs e)
		{
			...
			for (int i = m_animations[m_currentAnimation].parts.Count - 1; i >= 0; --i)
			{
				...
				newTime.Add(m_animations[m_currentAnimation].parts[i].SpriteTime.Text);
				RemoveSpriteToAnimation(m_animations[m_currentAnimation].parts[i]);
			}
			for (int i = 0; i < newList.Count; ++i)
			{
				AddSpriteToAnimation(newList[i], float.Parse(newTime[i]));
			}
		}
		...
		private void Save_Click(object sender, EventArgs e)
		{
			...
			using (Stream fileStream = saveFileDialog.OpenFile())
			{
				using (StreamWriter writer = new StreamWriter(fileStream))
				{
					...
					writer.Write("MULTI_TIMED_ANIM " + m_animations.Count + " " + texturesCount);

					int startIndex = 0;
					foreach (Animation animation in m_animations)
					{
						writer.Write("\n" + animation.AnimationName + " " + startIndex + " " + animation.parts.Count);
					...
					foreach (Animation animation in m_animations)
					{
						foreach (AnimationPart part in animation.parts)
						{
							...
							writer.Write("\n" + builder.ToString() + " " + part.SpriteTime.Text);
						}
					}
				}
			}
			...
			using (Stream stream = openFileDialog.OpenFile())
			{
				using (StreamReader reader = new StreamReader(stream))
				{
					...
					for (int i = 0; i < animationsNumber; ++i)
					{
						...
						AddAnimation(animationLineSplit[0]);
						animationsStartIndex.Add(int.Parse(animationLineSplit[1]));
						animationsSpriteNumber.Add(int.Parse(animationLineSplit[2]));
					}

					List<string> spritesNames = new List<string>();
					List<string> spritesTimes = new List<string>();

					for (int i = 0; i < spritesNumber; ++i)
					{
						string[] spriteInfo = reader.ReadLine().Split(" ");
						spritesNames.Add(spriteInfo[0]);
						spritesTimes.Add(spriteInfo[1]);
					}
					...
					for (int i = 0; i < m_animations.Count; ++i)
					{
						...
						for (int j = animationsStartIndex[i]; j < animationsStartIndex[i] + animationsSpriteNumber[i]; ++j)
							AddSpriteToAnimation(directoryEnginePath + spritesNames[j], float.Parse(spritesTimes[j]));
					}
				}
			}
		}
		...
		private void AddAnimation(string animationName)
		{
		...
		private void AddSpriteToList(string[] files, char splitSymbol)
		{
			foreach (var file in files)
			{
				...
				btn.TabIndex = 0;
				btn.Click += (object? sender, EventArgs e) => AddSpriteToAnimation(name, 0.1f);
		...
		private void AddSpriteToAnimation(string name, float time)
		{
			if (!m_imagesDictionary.ContainsKey(name))
			{
			...
			textBox.Location = new Point(250, YPosition);
			textBox.Size = new Size(30, 30);

			panelAnim.Controls.Add(textBox);
		...
		void RemoveSpriteToAnimation(AnimationPart part)
		{
			...
			for (int pos = 0; pos < m_animations[m_currentAnimation].parts.Count; ++pos)
			{
				...
				m_animations[m_currentAnimation].parts[pos].SpriteTime.Location = new Point(m_animations[m_currentAnimation].parts[pos].SpriteTime.Location.X, pos * 30 + panelAnim.AutoScrollPosition.Y);
			}
		...
		void SelectAnimation(Animation anim)
		{
			...
			foreach (AnimationPart part in m_animations[m_currentAnimation].parts)
			{
				...
				panelAnim.Controls.Add(part.SpriteTime);
			}
		}
		...
		void AnimationThreadFunction()
		{
			...
			while (m_threadRunning)
			{
				try
				{
					...
					if (timer > float.Parse(m_animations[m_currentAnimation].parts[currentSprite].SpriteTime.Text))
					{
						timer = 0f;
