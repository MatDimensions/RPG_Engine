	...
	public partial class TimedAnimationTool : Form
	{
		...
		private void InvertAnimationList_Click(object sender, EventArgs e)
		{
			...
			for (int i = m_animationParts.Count - 1; i >= 0; --i)
			{
				...
				newTime.Add(m_animationParts[i].SpriteTime.Text);
				RemoveSpriteToAnimation(m_animationParts[i]);
			}
		...
		private void Save_Click(object sender, EventArgs e)
		{
			...
			using (Stream fileStream = saveFileDialog.OpenFile())
			{
				...
				{
					writer.Write("TIMED_ANIM " + m_animationParts.Count);
		...
		private void Load_Click(object sender, EventArgs e)
		{
			...
			using (Stream stream = openFileDialog.OpenFile())
			{
				using (StreamReader reader = new StreamReader(stream))
				{
					...
					for (int i = 0; i < spritesNames.Count; ++i)
					{
						AddSpriteToAnimation(directoryEnginePath + spritesNames[i], float.Parse(spritesTimes[i]));
					}
				}
			}
		}
		...
		void RemoveSpriteToAnimation(AnimationPart part)
		{
			...
			for (int pos = 0; pos < m_animationParts.Count; ++pos)
			{
				...
				m_animationParts[pos].SpriteTime.Location = new Point(m_animationParts[pos].SpriteTime.Location.X, pos * 30 + panelAnim.AutoScrollPosition.Y);
			}
		...
		void AnimationThreadFunction()
		{
			...
			while (m_threadRunning)
			{
				try
				{
					...
					if (timer > float.Parse(m_animationParts[currentSprite].SpriteTime.Text))
					{
						timer = 0f;
